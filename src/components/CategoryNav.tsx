...
export default function CategoryNav({ categorias }: Props) {
  ...
  return (
    <div className="sticky top-0 z-10 bg-[#0a0a0a] border-b border-yellow-500 mb-10">
      ...
      <details className="md:hidden py-2 px-3 cursor-pointer select-none">
        ...
        <div className="flex flex-col gap-2 mt-4 items-center">
          {categorias.map((cat) => (
            <a
              ...
              href={`#${cat.id}`}
              className={clsx(
                "text-sm font-medium px-4 py-0 rounded-full border border-yellow-500 transition-all w-fit ",
                cat.id === activeId
